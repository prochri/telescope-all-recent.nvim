...
function Helpers.winsInTab(child, tab)
  tab = tab or "_G.TelescopeAllRecent.state.activeTab"
...
Helpers.expect.buf_width = MiniTest.new_expectation("variable in child process matches", function(child, field, value)
  return Helpers.expect.equality(
    child.lua_get("vim.api.nvim_win_get_width(_G.TelescopeAllRecent.state." .. field .. ")"),
    value
  )
end, errorMessage)
...
Helpers.expect.config = MiniTest.new_expectation("config option matches", function(child, field, value)
  return Helpers.expect.global(child, "_G.TelescopeAllRecent.config." .. field, value)
end, errorMessage)

Helpers.expect.config_type = MiniTest.new_expectation("config option type matches", function(child, field, value)
  return Helpers.expect.global(child, "type(_G.TelescopeAllRecent.config." .. field .. ")", value)
end, errorMessage)

Helpers.expect.state = MiniTest.new_expectation("state matches", function(child, field, value)
  return Helpers.expect.global(child, "_G.TelescopeAllRecent.state." .. field, value)
end, errorMessage)

Helpers.expect.state_type = MiniTest.new_expectation("state type matches", function(child, field, value)
  return Helpers.expect.global(child, "type(_G.TelescopeAllRecent.state." .. field .. ")", value)
end, errorMessage)
